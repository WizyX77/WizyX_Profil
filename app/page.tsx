          ...
          <>
            ...
            >
              <div className="flex justify-between items-start pt-2">
                <div>
                  <div>
                    <h2 className="text-2xl ml-1"> 𓆩♡𓆪・ {!isLoading && discordData?.discord_user.global_name} </h2>
                    <span className="text-[#a7a7a7] ml-3">K. Adı: {!isLoading && `@${discordData?.discord_user.username}`}</span>
                    <span className="text-[#a7a7a7] ml-1"> ・ Bot/Web Developer </span>
                  </div>

                  <p className="mt-4 text-[#a7a7a7]"> 𓆩♡𓆪 Merhaba Ben Efe, 17 Yaşındayım Ve 3 Yıldır Yazılım İle İlgileniyorum. Bu Kadar Yeter :D <br/>
                    Hakkımda ki Diğer Bilgileri de Siteye Göz Atarak Öğrenebilirsin. (Bot/Website Yapılır) 
                  </p>
                </div>
                ...
                <div className="w-96 lg:w-48 indicator flex justify-end">
                  {!isLoading && (
                    <>
                      <Image src={`https://cdn.discordapp.com/avatars/${discordData?.discord_user.id}/${discordData?.discord_user.avatar}?size=1024`} alt="@_wizyx" height={1024} width={1024} className="rounded-full lg:w-20 lg:h-20 h-34 w-96" />
