    ...
    <div className='container mx-auto bg-zinc-900/30 w-11/12 sm:w-9/12 md:w-7/12 rounded-md flex flex-col-reverse my-16 py-10 px-8 justify-between md:flex-row md:items-center'>
      <div className='md:w-8/12'>
        <div className='space-y-2'>
          <h1 className='text-2xl font-semibold'> Selam, Ben <span className='text-2xl font-semibold text-violet-500 hover:opacity-75 cursor-pointer'>@_wizyx</span> </h1>
          <ul className='flex flex-row font-medium gap-x-1'>
            <li key={1}> He/Him  ・       </li>
            <li key={2}> Junior Developer.</li>
          </ul>
        </div>
              ...
              <>
                ...
                <div className='truncate hover:text-clip'>
                  <div>
                    ...
                    <p>{discord.data.spotify.artist.includes(';') ? discord.data.spotify.artist.split(';')[0] : discord.data.spotify.artist} - <b>{discord.data.spotify.song}</b> Dinliyor. <b></b></p>
                    </div>
                  </div>
                </div>
              </>
      ...
      <div className='avatar'>
        {discord ? (
          ...
          <div className={'w-36 rounded-full mb-4 md:mb-0 ring ring-primary ring-offset-base-100 ring-offset-2'}>
            <Image className='rounded-full hover:opacity-75 transition duration-700' src={'https://media.tenor.com/bujRPc5A7jAAAAAM/frog-and-dino-frog.gif'} alt='avatar' width={1024} height={1024} />
          </div>
        )}
      </div>
    </div>
